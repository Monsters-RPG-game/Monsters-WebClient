...
const Portal: React.FC<IPortalProps> = ({ className, children, openButton, handleClose, triggerFn, deleteAccountHandler, isPortalOpen, confirmButtonLabel, cancelButtonLabel, deleteButtonLabel }) => {
  ...
  return (
    <dialogs.AlertDialog open={isPortalOpen}  >
      ...
      <dialogs.AlertDialogContent className={className as string}>
        ...
        <div className={cofirmDialog ? 'h-[30%]' : 'h-[100%] relative '}>{children}
          {errorMsg &&
        ...
        <dialogs.AlertDialogCancel className=" absolute top-0 right-0 bg-transparent text-slate-400 my-2 border-none hover:bg-transparent hover:text-slate-300" onClick={() => { handleClose(); setConfrimDialog(false); setErrorMsg(''); }}>
          &#x2716;
        ...
        {!cofirmDialog ? (<div className="flex  flex-col-reverse  gap-2 mt-3 md:flex-row">
          ...
          <div className='flex flex-col w-3/4 mx-auto md:flex-row md:justify-start md:px-6'>
            {deleteButtonLabel && <dialogs.AlertDialogAction className=" bg-rose-800 hover:bg-rose-700 my-2" onClick={() => deleteAccountHandler(confirmDeleteAccountHandler(true, setTodeleteAccount))}>
              {deleteButtonLabel}
